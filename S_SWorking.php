...
mysql_select_db($database_MyHomeStay_System, $MyHomeStay_System);
$query_Recordset1 = sprintf("SELECT * FROM staff natural join shift WHERE S_IC = %s and (Work_Date BETWEEN %s and %s) ORDER BY Work_Date", GetSQLValueString($colname_Recordset1, "text"),GetSQLValueString($colname1_Recordset1, "text"),GetSQLValueString($colname2_Recordset1, "text"));
$Recordset1 = mysql_query($query_Recordset1, $MyHomeStay_System) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
...
mysql_select_db($database_MyHomeStay_System, $MyHomeStay_System);
$query_Recordset3 = sprintf("SELECT * FROM staff natural join shift WHERE S_IC = %s and (Work_Date BETWEEN %s and %s) order by Work_Date", GetSQLValueString($colname_Recordset3, "text"),GetSQLValueString($colname1_Recordset3, "text"),GetSQLValueString($colname2_Recordset3, "text"));
...
$date2 = new DateTime($row_Recordset3['Work_Checkout']); $diff = $date2->diff($date1); echo $hour=$diff->format('%h');
 ...
 ?></td>
                      ...
                      </tr><?php } while ($row_Recordset3 = mysql_fetch_assoc($Recordset3)); ?><tr style="border-top: 1px solid #CCC;">
...
<div id="homestay-sec" class="container set-pad" ><div class="row text-center " >
  <div class="col-lg-8 col-lg-offset-2 col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
                 ...
                 <p>
    <form name="form1" method="post" action="S_SWorking.php">
                  <p>
                    <label for="txtname"></label>
  <select name="txtname" id="txtname" class="form_control form_control1 textfieldw" style="cursor:pointer">
   <?php echo isset($_POST['txtname'])?$var=mysql_escape_string($row_Recordset4['S_IC']):$var=""; ?>
    <?php
do {  
?>
